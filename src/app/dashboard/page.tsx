...
export default async function DashboardPage() {
  ...
  const statCards = [
    { icon: '📢', label: 'Total Channels', value: stats.totalChannels, color: 'hsl(210 85% 60%)' },
    { icon: '📝', label: 'Posts Today', value: stats.postsToday, color: 'hsl(145 65% 50%)' },
    { icon: '⏰', label: 'Scheduled', value: stats.scheduledPosts, color: 'hsl(38 95% 55%)' },
    { icon: '📊', label: 'Total Posts', value: stats.totalPosts, color: 'hsl(250 85% 65%)' },
  ]
          ...
          <div key={card.label} className="glass rounded-2xl p-5 hover:scale-[1.02] transition-transform duration-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: 'hsl(215 15% 55%)' }}>{card.label}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="glass rounded-2xl p-6">
        ...
        {recentPosts.length === 0 ? (
          <div className="text-center py-12" style={{ color: 'hsl(215 15% 55%)' }}>
            ...
            <p>No posts yet. Start composing!</p>
          </div>
        ) : (
